...
- (NSImage*) upsideDownShadowed: (float) fraction {
    ...
    [NSGraphicsContext saveGraphicsState];
    NSImage* r = [NSImage.alloc initWithSize: self.size];
    r.flipped = true;
    [r lockFocus];
    NSColor* c = [NSColor colorWithCalibratedWhite: 1.0 alpha: 0.2];
    NSGradient* g = [NSGradient.alloc initWithStartingColor: c endingColor: NSColor.clearColor];
    NSRect rc = NSMakeRect(0, 0, self.size.width, self.size.height * fraction);
    [g  drawInRect: rc angle: 90.0];
    [self drawInRect: rc fromRect: NSZeroRect operation: NSCompositeSourceIn fraction: 1.0];
    [r unlockFocus];
    [NSGraphicsContext restoreGraphicsState];
    return r;
}
