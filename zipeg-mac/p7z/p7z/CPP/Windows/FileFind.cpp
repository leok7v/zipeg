...
#ifdef __APPLE_CC__
  AString Awildcard;
  if (!ConvertUnicodeToUTF8(wildcard, Awildcard)) {
      return false;
  }
#else 
...
#ifdef __APPLE_CC__
      bret = ConvertUTF8ToUnicode(fileInfo0.Name, fileInfo.Name);
#else
...
#ifdef __APPLE_CC__
  AString name;
  if (!ConvertUnicodeToUTF8(wildcard, name)) {
    return false;
  }
#else
...
#ifdef __APPLE_CC__
  AString Aname;
  if (!ConvertUnicodeToUTF8(name, Aname)) {
    return false;
  }
#else
