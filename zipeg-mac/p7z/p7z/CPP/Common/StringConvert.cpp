...
/*
  if (!srcString.IsEmpty())
  {
    ...
    const char * path = &srcString[0];
      printf("MultiByteToUnicodeString ");
      hexdump(path);
// FIXME    size_t n = strlen(path);
//      int encoding = kCFStringEncodingUTF8;
      int encoding = kCFStringEncodingWindowsLatin2; // cp1250
      
    CFStringRef cfpath = CFStringCreateWithCString(NULL, path, encoding);
//  this is BAD place to normalize:
//  http://forums.macrumors.com/archive/index.php/t-862335.html
    if (cfpath)
    {
