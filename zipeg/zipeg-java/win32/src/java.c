...
#ifdef _DEBUG
jboolean _launcher_debug = JNI_TRUE;
#else
...
static char* SelectVersion(int argc, char **argv, char **main_class);
static jboolean ParseArguments(int *pargc, char ***pargv, char **pjarfile,
			       char **pclassname, int *pret, const char *jvmpath);
static jboolean InitializeJVM(JavaVM **pvm, JNIEnv **penv,
			      InvocationFunctions *ifn);
static jstring NewPlatformString(JNIEnv *env, char *s);
static jobjectArray NewPlatformStringArrayFromUTF8(JNIEnv *env, char **strv, int strc);
...
static void InstallLatestJRE();
static void ManualDownload();
...
static int argc_wo_jar; // number of arguments before -jar foo.jar params
...
static int CheckFile(const char* path, const char* tail) {
    int r;
    char file[MAXPATHLEN];
    strcat(strcpy(file, path), "/bin/javaw.exe");
    r = FileExists(file); 
    return r;
}

static int
IsGoodJRE(const char* path) {
    if (!CheckFile(path, "/bin/javaw.exe")) 
        return 0;
    if (!CheckFile(path, "/bin/jawt.dll")) 
        return 0;
    if (!CheckFile(path, "/bin/client/jvm.dll")) 
        return 0;
    if (!CheckFile(path, "/lib/rt.jar")) 
        return 0;
    if (!CheckFile(path, "/bin/javaw.exe")) 
        return 0;
    if (!CheckFile(path, "/lib/charsets.jar")) 
        return 0;
    return 1;
}

static
const char* getProgramPathAndSetWokingDir(const char* argv0) {
    const char * pp;
    char* name;
    char cd[MAX_PATH + 1024];
    if (argv0[0] != '\"') {
        pp = argv0;
    } else {
        char* dup = strdup(&argv0[1]);
        dup[strlen(dup) - 1] = 0;
        pp = dup;
    }
    strcpy(cd, pp);
    ...
    strcat(cd, "\\");
    strcat(cd, pp);
//  trace("getProgramPathAndSetWokingDir=%s\n", cd);
    return strdup(cd);
}

static int flag(int argc, char** argv, const char* s) {
    int i;
    for (i = 0; i < argc; i++) {
        if (stricmp(s, argv[i]) == 0) {
            return 1;
        }
    }
    return 0;
...
{
    int i = 0;
    const char* argv0 = getProgramPathAndSetWokingDir(argv[0]);
    char *jarfile = 0;
    char *classname = 0;
    char *s = 0;
    ...
    int zlen = strlen("/zipeg.exe");
    int alen = strlen(argv0);
    int do_remove  = flag(argc, argv, "-uninstall-remove");
    int do_install = !do_remove && alen > zlen && 
        (stricmp(argv0 + (alen - zlen), "/zipeg.exe") != 0 &&
         stricmp(argv0 + (alen - zlen), "\\zipeg.exe") != 0 ||
         flag(argc, argv, "-install") || flag(argc, argv, "--install")
         );
    int do_uninstall = !do_install && flag(argc, argv, "-uninstall");
#ifdef _DEBUG
    if (flag(argc, argv, "--run")) {
        do_install = 0;
    }
#endif
    if (getenv("_JAVA_LAUNCHER_DEBUG") != 0) {
	_launcher_debug = JNI_TRUE;
	trace("----_JAVA_LAUNCHER_DEBUG----\n");
    }
    if (do_remove) {
        removeAll(argv0);
        ExitProcess(0);
    ...
    jrepath[0] = 0;
    while (jrepath[0] == 0) {
        char* jre = SelectVersion(argc, argv, &main_class);
        if (jre != NULL && IsGoodJRE(jre)) {
            strcpy(jrepath, jre);
        } else {
            // usual suspects:
            int k;
            char* programfiles = getSpecialFolderLocation(CSIDL_PROGRAM_FILES);
            for (k = 0; jre_locations[k] != NULL; k++) {
                char loc[MAXPATHLEN] = {0};
                strcpy(loc, programfiles);
                strcat(loc, "\\");
                strcat(loc, jre_locations[k]);
                trace("%d %s\n", IsGoodJRE(loc), loc);
                if (IsGoodJRE(loc)) {
                    strcpy(jrepath, loc);
                    break;
...
/*
      ...
      trace("%ld micro seconds to LoadJavaVM\n",
...
*/
    ...
    SetClassPath(s);
#endif
/*  no needed but good to know
    {
        char jars[MAXPATHLEN + 20];
        strcpy(jars, jrepath);
        strcat(jars, "\\lib\\*");
        SetClassPath(jars);
    }
*/
    {   // add local *.jar to the classpath if we have any
        char jars[MAXPATHLEN + 20];
        strcpy(jars, jrepath);
        strcat(jars, "\\lib\\*");
        SetClassPath(jars);
...
// -version:1.5* -Xmx512m -jar zipeg.jar

static
char** prepend(int* argc, char** argv) {
    enum { MB = 1024*1024 };
    int i, n, new_argc;
    char mem[128] = "-Xmx512m";
    char** new_argv;
    char* prefix[] = {
        "-version:1.6.0_18+",
        "-splash:win.splash.gif",
        mem,
	"-ea",
//	"-Xincgc",
	"-client",
//	"-Dswing.aatext=false",
    ...
    mb = (int)(ms.ullTotalPhys / MB);
    if (mb >= 512) {
        // XP has about 200MB footprint
        mb = (mb - 200) * 3 / 4;
    } else {
        mb = 128;
    }
    ...
    sprintf(mem, "-Xmx%dm", mb);
    n = sizeof(prefix) / sizeof(prefix[0]);
    new_argc = *argc + n;
    new_argv = JLI_MemAlloc((new_argc + 1) * sizeof(char*));
    new_argv[0] = argv[0];
    for (i = 0; i < n; i++) {
        new_argv[i + 1] = prefix[i];
    }
    argc_wo_jar = *argc + 1; // including version
    for (i = 1; i < *argc; i++) {
        new_argv[i + n] = argv[i];
    }
/*
...
JavaMain(void * _args)
{
     ...
     */
    char * message = 
        "Zipeg installation has been corrupted.\n"
        "Please download latest stable version from:\n"
        "http://www.zipeg.com\n"
        "Zipeg will exit now.";
...
/*
	...
	trace("%ld micro seconds to InitializeJVM\n",
	       (long)(jint)Counter2Micros(end-start));
*/
    }
...
*/
	...
	if(mainClass == NULL) { /* exception occured */
	    static char buf[256];
	    ReportExceptionDescription(env);
            sprintf(buf, "Could not find %s class.  Program will exit.", classname);
            message = buf;
	    goto leave;
	}
    ...
    mainArgs = NewPlatformStringArrayFromUTF8(env, argv, argc);
    if (mainArgs == NULL) {
	ReportExceptionDescription(env);
	goto leave;
    }
...
CheckJvmType(int *pargc, char ***argv, jboolean speculative) {
    ...
    *pargc = newArgvIdx;
    ...
    case VM_WARN:
        if (!speculative) {
	    trace("Warning: %s VM not supported; %s VM will be used\n", 
...
SelectVersion(int argc, char **argv, char **main_class)
{
	...
	*main_class = JLI_StringDup(info.main_class);
    ...
    if (_launcher_debug) {
/*
        trace("JRE-Version = %s, JRE-Restrict-Search = %s Selected = %s\n",
 ...
 */
	    ...
	    trace("%s full version \"%s\"\n", progname, FULL_VERSION);
	    *pret = 0;
	    return JNI_FALSE;
	    ...
	    /* No longer supported */
	    trace("Warning: %s option is no longer supported.\n",
		    arg);
 ...
 */
static jobjectArray
NewPlatformStringArrayFromUTF8(JNIEnv *env, char **strv, int strc)
{
    jarray cls;
    jarray ary;
    int i;
...
LoadClass(JNIEnv *env, char *name)
{
	...
	*s++ = (c == '.') ? '/' : c;
    ...
    if (_launcher_debug) {
/*
	jlong end   = CounterGet();
	trace("%ld micro seconds to load main class\n",
	       (long)(jint)Counter2Micros(end-start));
	trace("----_JAVA_LAUNCHER_DEBUG----\n");
*/
    }
...
PrintUsage(void)
{
    ...
    trace("Usage: %s [-options] class [args...]\n"
	"           (to execute a class)\n"
...
ReadKnownVMs(const char *jrepath, char * arch, jboolean speculative)
{
    ...
    while (fgets(line, sizeof(line), jvmCfg) != NULL) {
        ...
        if (line[0] == '#')
            continue;
        if (line[0] != '-') {
            trace("Warning: no leading - on line %d of `%s'\n",
                    lineno, jvmCfgName);
        }
        ...
        if (*tmpPtr == 0) {
            trace("Warning: missing VM type on line %d of `%s'\n",
                    lineno, jvmCfgName);
        } else {
            ...
            *tmpPtr++ = 0;
            ...
            if (*tmpPtr == 0) {
                ...
                if (!strncmp(tmpPtr, "KNOWN", strlen("KNOWN"))) {
                    ...
                    if (*tmpPtr == 0) {
                        trace("Warning: missing VM alias on line %d of `%s'\n",
                                lineno, jvmCfgName);
                    } else {
                    ...
                    if (*tmpPtr == 0) {
                        trace("Warning: missing server class VM on line %d of `%s'\n",
                                lineno, jvmCfgName);
                    } else {
                    ...
                    trace("Warning: unknown VM type on line %d of `%s'\n",
                            lineno, &jvmCfgName[0]);
                    vmType = VM_KNOWN;
                }
            }
...
*/
            ...
            case VM_IF_SERVER_CLASS:
                ...
                if (_launcher_debug) {
/*
                    trace("    name: %s  vmType: %s  server_class: %s\n", 
...
/*
        ...
        trace("%ld micro seconds to parse jvm.cfg\n",
               (long)(jint)Counter2Micros(end-start));
*/
    }
    return cnt;
...
static 
int KnownVMIndex(const char* name)
{
    int i;
...
//  MessageBox(NULL, buf, "Zipeg: trace (java.c)", MB_OK);
#endif
}

void exitProcess(int exitcode) {
#ifdef _DEBUG
    RaiseException(DBG_CONTROL_BREAK, EXCEPTION_NONCONTINUABLE, 0, NULL);
...
static
void* LoadFromResource(int id, const char* type, int* size) {
    HGLOBAL h = NULL;
    HINSTANCE hInstance = GetModuleHandle(NULL);
    HRSRC hRes = FindResource(hInstance, MAKEINTRESOURCE(id), type);
    if (hRes == NULL) {
        return NULL;
    }
    ...
    *size = SizeofResource((HINSTANCE)hInstance, hRes);
...
int WriteMemoryToFile(const char* filename, void* p, int size) {
    ...
    if (file == INVALID_HANDLE_VALUE) {
        trace("WriteMemoryToFile failed to create file %s %d\n", filename, GetLastError());
        return 0;
    }
    r = WriteFile(file, p, size, &written, NULL) && ((int)written == size);
    if (!r) {
        trace("WriteMemoryToFile failed to write file %s\n", filename);
    }
    CloseHandle(file);
    if (!r) {
        DeleteFile(filename);
...
void* ReadFileIntoMemory(const char* filename, int * size) {
    ...
    if (file == INVALID_HANDLE_VALUE) {
        trace("ReadFileIntoMemory failed to open file %s %d\n", filename, GetLastError());
        return NULL;
    }
    n = GetFileSize(file, NULL);    
    p = malloc(n + 1);
    if (p == NULL) {
        trace("ReadFileIntoMemory failed to allocate memory for file %s %d\n", filename, GetLastError());
        return NULL;
    }
    r = ReadFile(file, p, n, &read, NULL) && ((int)read == n);
    if (!r) {
        trace("ReadFileIntoMemory failed to read file %s\n", filename);
    }
    CloseHandle(file);
    if (!r) {
        free(p);
...
int CopyFileFromResource(const char* filename, int id, const char* type) {
    int size = 0;
    void* p = LoadFromResource(id, type, &size);
    if (p == NULL || size == 0) {
        trace("CopyFileFromResource failed to load resource %d\n", id);
        return 0;
    }
    return WriteMemoryToFile(filename, p, size);
}
...
static void ManualDownload() {
    ShellExecute(NULL, "open", "http://java.com/en/download/index.jsp?cid=jdp98583", NULL, NULL, SW_SHOW);
    MessageBox(NULL, "Please install Java from http://java.com first,\n"
        "than re-run Zipeg installer.", "Zipeg needs Java", MB_OK|MB_ICONERROR);
    ExitProcess(0);
}
...
static const char iftw_zip[] = "jre-6-windows-i586-iftw.zip";
static const char iftw_exe[] = "jre-6-windows-i586-iftw.exe";

static
int shellExecEx(const char* exe, const char* params) {
    SHELLEXECUTEINFO ShExecInfo = {0};	// must include shellapi.h 
    ShExecInfo.cbSize = sizeof(SHELLEXECUTEINFO);
    ShExecInfo.fMask = SEE_MASK_NOCLOSEPROCESS;	// ShExecInfo.hProcess == null if SEE_MASK_NOCLOSEPROCESS not set
    ...
    if ((int)ShExecInfo.hInstApp <= 32 || ShExecInfo.hProcess == INVALID_HANDLE_VALUE) {
        return FALSE;
    }
    { 
        DWORD retVal = WaitForSingleObject(ShExecInfo.hProcess, INFINITE);
...
static void InstallLatestJRE() {
    ...
    void * buf = NULL;
    int size = 0;
    int res = DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(idd_install_jre), NULL, dlgProc);
    if (res != IDYES) {
        ManualDownload();
    }
    if (!CopyFileFromResource(iftw_zip, idr_iftw, RT_RCDATA)) {
        ManualDownload();
    }
    buf = JLI_JarUnpackFile(iftw_zip, iftw_exe, &size);
    if (buf == NULL || size <= 0) {
        ManualDownload();
    }
    if (!WriteMemoryToFile(iftw_exe, buf, size)) {
        DeleteFile(iftw_zip);
        ManualDownload();
    }
    DeleteFile(iftw_zip);
//  "jre-6-windows-i586-iftw.exe" /s /v "/qn ADDLOCAL=ALL NETSCAPE=0 MOZILLA=0 IEXPLORER=0 "
//  "INSTALLDIR=C:\temp\test REBOOT=Suppress /L .\jre-setup.log JAVAUPDATE=0 EULA=1"
/*
jre-6-windows-i586-iftw.exe /s /quiet /norestart /passive /qr /qn AgreeToLicense=YES ADDLOCAL=ALL REBOOT=Suppress EULA=1 WEBSTARTICON=0 JAVAUPDATE=0 JU=0 SYSTRAY=0
*/
    if (!CreateProcessA(iftw_exe,	/* executable name */
        "/s /quiet /norestart /passive /qr /qn AgreeToLicense=YES ADDLOCAL=ALL REBOOT=Suppress EULA=1 WEBSTARTICON=0 JAVAUPDATE=0 JU=0 SYSTRAY=0", // /L .\\jre-setup.log 
        ...
        trace("CreateProcess(%s, ...) failed: %d\n", iftw_exe, GetLastError());
        if (shellExecEx(iftw_exe, /* executable name */
            "/s /quiet /norestart /passive /qr /qn AgreeToLicense=YES ADDLOCAL=ALL REBOOT=Suppress EULA=1 WEBSTARTICON=0 JAVAUPDATE=0 JU=0 SYSTRAY=0")) {
            return;
        }
        ManualDownload();
//      DeleteFile(iftw_exe);
    ...
    DeleteFile(iftw_exe);
    if (exitCode != 0) {
	trace("installer failed %d.", exitCode);
        ManualDownload();
    }
}
