...
public final class Settings extends JTabbedPane {
    ...
    public static void showPreferences() {
        final long oldFlags = Flags.getFlags();
        JDialog d = Util.createDocumentModalDialog(MainFrame.getTopFrame());
        Dialogs.setModalSheet(d);
        final Settings s = new Settings();
        d.setTitle(Util.isMac ? "Zipeg Preferences" : "Zipeg Options");
        d.getContentPane().setLayout(new BorderLayout());
        d.getContentPane().add(s, BorderLayout.CENTER);
        d.getContentPane().add(s.createButtons(d), BorderLayout.SOUTH);
        d.pack();
        d.setResizable(false);
        Dialogs.show(d);
        // invoke later to enable Zipeg to be repainted
    ...
    private final class General extends BasePanel {
        ...
        General() {
            ...
            dirsFirst = new JCheckBox("Group folders on top of list view");
            ...
            String trash = Util.isWindows ? "Recycle" : "Trash";
            useTrashBin = new JCheckBox("Use " + trash + " Bin (when overwriting files)");
            useTrashBin.setSelected(!Flags.getFlag(Flags.DONT_USE_TRASH_BIN));
            useTrashBin.setMnemonic('B');
            add(useTrashBin);

            showPasswords = new JCheckBox("Show passwords");
            showPasswords.setSelected(Flags.getFlag(Flags.SHOW_PASSWORDS));
            showPasswords.setMnemonic('w');
            add(showPasswords);
            ...
            updates = new JCheckBox("Check for updates (once a month)");
            updates.setSelected(!Flags.getFlag(Flags.DONT_CHECK_FOR_UPDATES));
        ...
        protected final void saveSettings() {
            ...
            if (showPasswords.isSelected()) {
                Flags.addFlag(Flags.SHOW_PASSWORDS);
            } else {
                Flags.removeFlag(Flags.SHOW_PASSWORDS);
            }
            if (useTrashBin.isSelected()) {
                Flags.removeFlag(Flags.DONT_USE_TRASH_BIN);
            } else {
                Flags.addFlag(Flags.DONT_USE_TRASH_BIN);
            }
            if (updates.isSelected()) {
                Flags.removeFlag(Flags.DONT_CHECK_FOR_UPDATES);
            } else {
                Flags.addFlag(Flags.DONT_CHECK_FOR_UPDATES);
            }
    ...
    private final class FileSettings extends BasePanel {
        ...
        FileSettings() {
            if (fa.isAvailable()) {
                ...
                label.setText("<html><body><br>On Mac OS X prio 10.4 use <b>Finder</b> " +
                        "to associate Zipeg with archives.</body></html>");
                setLayout(new BoxLayout(this, BoxLayout.Y_AXIS));
                add(label);
            }
        }
    ...
    private final class Advanced extends BasePanel {
        ...
        Advanced() {
            ...
            String mydocs = Util.isMac ? "<html><body><u>D</u>ocuments folder (<b>" +
                                           Util.getDocuments() + "</b>)</body></html>" :
                                           "<html><body><b>My <u>D</u>ocuments</b> folder</body></html>";
...
*/
        ...
        protected boolean checkSettings() {
            ...
            if (!appendArchiveName.isSelected() && (desk.isSelected() || docs.isSelected())) {
                ...
                panel.add(cbx);
                // This is two-story message box. Nasty. But using MessageBox.show will
                // not work at all because it checks on a singularity of a Dialog.
                // Keep JOptionPane.showConfirmDialog here for now, till better idea.
                int i = JOptionPane.showConfirmDialog(MainFrame.getTopFrame(), panel,
                    "Zipeg: Extracting Too Many Files",
