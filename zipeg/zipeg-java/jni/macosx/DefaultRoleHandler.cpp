...
extern "C" {
  ...
  JNIEXPORT jstring Java_com_zipeg_DefaultRoleHandler_getApplicationForFile(JNIEnv * env,
  								jobject _this,
                                                                jstring filename, jint role) {
      jstring app = null;
      const char* utf = env->GetStringUTFChars(filename, null);
      FSRef fileFSRef = {0};
      Boolean isDirectory = false;
      if (FSPathMakeRef((UInt8*)utf, &fileFSRef, &isDirectory) == noErr) {
        FSRef appFSRef;
        if (::LSGetApplicationForItem(&fileFSRef, role, &appFSRef, null) == noErr) {
            char path[4*1024] = {0};
            if (FSRefMakePath(&appFSRef, (UInt8*)path, sizeof(path)) == noErr) {
              app = env->NewStringUTF(path);
            }
        }
      }
