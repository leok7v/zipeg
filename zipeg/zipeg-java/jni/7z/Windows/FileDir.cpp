...
bool MyMoveFile( LPCTSTR fn1, LPCTSTR fn2 ) {
  ...
  return true;
#else
  return ::MoveFileEx(fn1, fn2, MOVEFILE_REPLACE_EXISTING|MOVEFILE_COPY_ALLOWED) != 0;
#endif
}
...
{
    HANDLE h = CreateFileW(fileName, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
    bool b = SetFileTime(h, creationTime, lpLastAccessTime, lpLastWriteTime) != 0;
    ::CloseHandle(h);
    return b;
}
#else
...
{
#ifdef _WINDOWS
  return ::CreateDirectoryA(pathName, NULL) != 0;
#else
...
#ifdef _WINDOWS
  if(fileInfo.IsDirectory())
    return RemoveDirectoryWithSubItems(pathPrefix + fileInfo.cFileName);
  else
    return DeleteFileAlways(pathPrefix + fileInfo.cFileName);
#else
  if(fileInfo.IsDirectory())
...
{
    wchar_t buf[32*1024];
    GetTempPathW(sizeof(buf), buf);
    path = buf; // final '/' is needed
...
bool MyGetTempPath(UString &path)
{
    ...
    int n = strlen(tmpdir);
    wchar_t* wtmpdir = new wchar_t[n + 1];
    if (wtmpdir == NULL) {
        return false;
    }
    for (int i = 0; i < n; i++) {
...
#ifdef _WIN32
void MyGetCurrentDirectory(UString& dir) {
    wchar_t buf[32*1024];
    ::GetCurrentDirectoryW(sizeof(buf) / sizeof(wchar_t), buf);
    dir = buf;
}

bool MySetCurrentDirectory(const wchar_t* dir) {
