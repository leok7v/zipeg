...
static bool callBack(JNIEnv * env, jobject _this, const char* op, Int64 value, const wchar_t* str) {
    jclass cls = env->GetObjectClass(_this);
    if (cls == 0) {
    	return true;
    }
    jmethodID method = env->GetMethodID(cls, "callback", "(Ljava/lang/String;JLjava/lang/String;)Z");
    if (method == 0) {
      return true;
    }
    jstring o = env->NewStringUTF(op);
    jstring s = env->NewString((jchar*)str, wcslen(str));
    jboolean r = env->CallBooleanMethod(_this, method, o, value, s) != 0;
    return (bool)r;
}
    ...
    public CMyUnknownImp
{
    ...
    SingleExtractCallback(JNIEnv * env, jobject that, IInArchive *archive, const wchar_t * path) {
        _env = env;
        _that = that;
...
/*
        char buf[128] = {0};
        sprintf(buf, "SingleExtractCallback::SetTotal(%I64d)\n", total);
        OutputDebugStringA(buf);
*/
        if (!callBack(_env, _that, "setTotal", total, L"")) {
            return E_ABORT;
        }
        return S_OK;
    }
...
/*
        char buf[128] = {0};
        sprintf(buf, "SingleExtractCallback::SetCompleted(%I64d)\n", *value);
        OutputDebugStringA(buf);
*/
//	printf(">setCompleted 0x%08X %lld\n", (int)_that, *value);
        if (!callBack(_env, _that, "setCompleted", *value, L"")) {
//	printf("<setCompleted ABORT\n");
            return E_ABORT;
        }
//	printf("<setCompleted OK\n");
        return S_OK;
    }
...
static jlong extractItem(JNIEnv * env, jobject that, CArchiveLink* archiveLink, UInt32 ix,
  ...
  SingleExtractCallback *extractCallbackSpec =
      new SingleExtractCallback(env, that, archive, path);
...
static HRESULT updateArchive(const wchar_t *filePath, const wchar_t* pwd) {
    CUpdateOptions uo; // xxx??? = options.UpdateOptions;
    uo.MethodMode.Name = L"Zip"; // L"7z", L"BZip2", L"GZip", L"Tar", L"Zip"
    CProperty compress;
    compress.Name = L"X9";
    compress.Value = L"";
    uo.MethodMode.Properties.Add(compress);
    CProperty method;
    method.Name = L"M";
    method.Value = L"DEFLATE"; // L"COPY", L"DEFLATE64", L"BZIP2"
    uo.MethodMode.Properties.Add(method);
    CProperty em;
    em.Name = L"EM";
    em.Value = L"AES192"; // L"AES128", L"AES256"
    uo.MethodMode.Properties.Add(em);
    // see ZipHandlerOut.cpp line 272 SetProperties
    // and 7zHandlerOut.cpp line 975 SetProperties

    bool passwordIsDefined = pwd != null && pwd[0] != 0;

    COpenCallbackConsole openCallback;
    openCallback.OutStream = g_StdStream;
    openCallback.PasswordIsDefined = passwordIsDefined;
    openCallback.Password = passwordIsDefined ? pwd : L"";

    CUpdateCallbackConsole callback;
    callback.EnablePercents = false;
    callback.PasswordIsDefined = passwordIsDefined;
    callback.AskPassword = false;
    callback.Password = openCallback.Password;
    callback.StdOutMode = false;
    callback.Init(g_StdStream);

    CUpdateErrorInfo errorInfo;
    NUpdateArchive::CActionSet defaultActionSet = NUpdateArchive::kAddActionSet;
    // NUpdateArchive::kDeleteActionSet, NUpdateArchive::kUpdateActionSet;
    uo.UpdateArchiveItself = true;
    uo.ArchivePath.ParseFromPath(filePath);
    CUpdateArchiveCommand updateMainCommand;
    updateMainCommand.ActionSet = defaultActionSet;
    updateMainCommand.ArchivePath.ParseFromPath(filePath);
    uo.Commands.Add(updateMainCommand);

    const wchar_t* files[] = {
	    L"c:\\shell32_155.ico",
	    L"c:\\bin1\\sub\\zip.exe",
	    NULL
    };
    ...
    HRESULT result = UpdateArchive2(prefix, files, uo, errorInfo, &openCallback, &callback);
    return result;
}
...
//  updateArchive(L"c:\\test.zip", null); // L"123456"
    try {
  ...
  JNIEXPORT jlong JNICALL Java_com_zipeg_Z7_extractItem(JNIEnv * env,
    ...
    try {
      ...
      if (archiveLink != null) {
        ...
        int res = 0;
        jlong hr = extractItem(env, _this, archiveLink, ix, path.s, pwd.s, &res) & 0xFFFFFFFFLL;
        if (hr == 0) {
          hr = res;
        }
        return res;
