...
#define strmethod(name) _method(jstring, name)
...
method(getCurrentProcessId)(ENV) {
...
strmethod(getSpecialFolderLocation)(ENV, jint location) {
    wchar_t buf[16*1024] = {0};
    LPITEMIDLIST idl = NULL;
    HRESULT hRes = ::SHGetSpecialFolderLocation(NULL, location, &idl);
    if (SUCCEEDED(hRes)) {
	::SHGetPathFromIDListW(idl, buf);
	jstring r = env->NewString((const jchar*)buf, wcslen(buf));
	return r;
    } else {
	return null;
    }
...
voidmethod(outputDebugString)(ENV, jstring s) {
    jchar* buf = getString(env, s);
    if (buf != null) {
