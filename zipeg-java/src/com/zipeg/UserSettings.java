...
//      Debug.traceln("UserSettings.clear()");
    ...
    public static Map getAll() {
        synchronized (getUser()) {
            return Collections.unmodifiableMap(getUser());
        }
    }
    ...
    public static int getInt(String key, int value) {
        String r = (String)getUser().get(key);
        try {
            return r == null ? value : Integer.parseInt(r.trim(), 10);
    ...
    public static long getLong(String key, long value) {
        String r = (String)getUser().get(key);
        try {
            return r == null ? value : Long.parseLong(r.trim(), 10);
        } catch (NumberFormatException x) {
            return value;
        }
    }
    ...
    public static boolean getBoolean(String key, boolean value) {
        String r = (String)getUser().get(key);
        return r == null ? value : "true".equalsIgnoreCase(r.trim());
    }
...
//                      Debug.traceln("UserSettings.flushed");
                    ...
                    if (retry == 0) {
                        ...
                        Util.mkdirs(prefs.getParentFile());
                    }
                } finally {
                    Util.close(os);
                }
...
//                      Debug.traceln("UserSettings.loaded");
                    ...
                    return;
                } catch (Throwable e) { // IOException, IllegalArgumentException: Malformed \\uxxxx encoding.
                    if (retry == 0) {
                        Debug.printStackTrace(e);
                        if (Debug.isDebug()) {
                            throw new Error(e);
