...
//      Debug.traceln("new Z7 0x" + Integer.toHexString(hashCode()));
        if (archive != 0) {
        ...
        if (archive == 0) {
            throw new IOException("the file is corrupted or file format is unsupported");
        }
        try {
...
//      Debug.traceln("finalize Z7 0x" + Integer.toHexString(hashCode()));
        super.finalize();
    }

    public boolean callback(String op, long value, String s) {
        return progress == null || progress.progress(op, value, s);
    }
    ...
    public void extractEntry(ZipEntry e, File file, String pwd, Progress progress) throws IOException {
        if (!(e instanceof Zip7Entry)) {
            ...
            assert parent != null : "parent == null for file: " + file + " file exists=" + exists;
            if (!parent.isDirectory()) {
                Util.mkdirs(parent);
                // TODO: do it for all non-existing parents
                ZipEntry p = getEntry(new File(z7e.getName()).getParent());
                if (p != null) {
                    Util.setLastModified(parent, p.getTime());
                }
            }
            ...
            if (r != 0) {
                ...
                if (isEncrypted()) {
                    z7e.error = z7e.error == null ? "wrong password" : z7e.error + " or wrong password";
                }
                throw new IOException(z7e.error + " while extracting item\n\"" + e.getName() + "\"");
            }
        }
    }
    ...
    @SuppressWarnings("UnusedDeclaration") public class Zip7Entry extends ZipEntry {
        private long index;
        ...
        private long block;
        private long time7; // to avoid Java2Dos conversion
...
//          return "7za-win-i386-dbg";
    ...
    /* unused\de\spieleck\app\ngramj\lm\CategorizerImpl.java actually
       ...
       But it is heave machinery. For now the simplest approach is to resort
...
*/          // practically the IBM850 is better choice even when than defaultCharset
            ...
            return forName("IBM850"); // most common for Western European ZIPs
        }
    }
...
//                      Debug.traceln("charset = " + name);
    ...
    private String decodeFileName(String s) {
        try {
            if (!sbc) {
                // TODO: may be necessary to do UTF-16BE vs UTF-16LE
            ...
            // both UnsupportedEncodingException and ArrayIndexOutOfBoundsException
            // in sun.nio.cs.ext.EUC_TW$Decoder.decodeArrayLoop happens.
    ...
    private String decodeWindowsName(String s) {
        ...
        if (invalid) {
            ...
            if (ix > 0) {
                ...
                File test = new File(Util.getCacheDirectory(true), s);
                try {
