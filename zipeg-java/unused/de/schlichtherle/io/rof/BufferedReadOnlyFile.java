...
/**
 * This class implements a {@link ReadOnlyFile} in order to provide buffered
 * random read only access to another <code>ReadOnlyFile</code>.
 * <p>
...
public class BufferedReadOnlyFile extends FilterReadOnlyFile {
     ...
     */
    private static final int WINDOW_LEN = 4096;
     ...
     * to access the contents of the RAES file.
     * This method is only used if the constructor isn't called with a read only
    ...
    public int read() throws IOException {
        ...
        positionWindow();
        return window[(int) (fp++ % window.length)];
    }
    ...
    public int skipBytes(int n) throws IOException {
        ...
        if (fp >= length())
            return 0;
        final long remaining = length() - fp;
        if (n > remaining)
            n = (int) remaining;
        fp += n;
    ...
    /** Throws an IOException if this read only file has been closed. */
